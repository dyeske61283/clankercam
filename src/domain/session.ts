...
export class Session {
  ...
  get tokenUsage(): TokenUsage {
    const input = this.data.messages.reduce((sum, msg) => sum + (msg.tokenUsage?.input ?? 0), 0);
    const output = this.data.messages.reduce((sum, msg) => sum + (msg.tokenUsage?.output ?? 0), 0);
