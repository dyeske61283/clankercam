...
async function renderProjects() {
    ...
    list.innerHTML = projects.map(p => `
        <div class="card" onclick="showSessions('${p.id}', '${p.path}')">
...
async function showSessions(projectId, path) {
    showView('sessions');
    document.getElementById('project-name').textContent = `Project: ${path === 'unknown' ? projectId : path}`;
    ...
    list.innerHTML = sessions.map(s => `
        <div class="list-item" onclick="showInspector('${s.id}')">
            <div>
...
async function showInspector(sessionId) {
    showView('inspector');
    ...
    list.innerHTML = data.messages.map(m => `
        <div class="message ${m.type}">
            <div class="meta">${new Date(m.timestamp).toLocaleTimeString()} - ${m.type}</div>
            <div class="content">${formatContent(m.content)}</div>
            ${m.toolCalls.map(tc => `
                <div class="tool-call">
                    <strong>${tc.name}</strong>(${JSON.stringify(tc.args).substring(0, 100)}...)
